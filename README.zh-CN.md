...
### 为重度 AI 用户而生的 macOS 极速 Prompt / 代码片段面板

一款原生、本地优先的 macOS 桌面应用：全局快捷键唤出快捷面板，瞬间检索你的 **Prompt 库 / 代码片段 / 模板**，把内容一键送进 **ChatGPT、Claude、Cursor、Copilot、VS Code、终端或任意输入框**。
...
[**English**](README.md) · [**简体中文**](README.zh-CN.md) · [**FAQ**](FAQ.md) · [**文档**](docs/README.md) · [**LLM 索引**](llms.txt) · [**变更记录**](CHANGELOG.md) · [**贡献指南**](CONTRIBUTING.md)

<img src="frontend-draft/uploads/PromptPanel-panel-default.png" alt="PromptPanel 在编辑器上方唤出的快捷面板：搜索并粘贴 Prompt" width="780" />
...
PromptPanel **永远不会**加入云同步、团队协作、复杂工作流编排。这不是"以后再说"，而是"以后也不做"。这个工具就是单用户、纯本地、轻量快速，这才是它存在的理由。详见 [PRD §4.2](项目快贴-PRD.md)。

## 截图

| 快捷面板 | 主界面 / 库管理 |
|:---:|:---:|
| <img src="frontend-draft/uploads/PromptPanel-panel-default.png" alt="PromptPanel 快捷面板：搜索 Prompt 词条" width="380"/> | <img src="frontend-draft/uploads/PromptPanel-library.png" alt="PromptPanel 主窗口：项目与词条管理" width="380"/> |
| 紧凑模式 | 设置 |
| <img src="frontend-draft/uploads/PromptPanel-panel-min.png" alt="PromptPanel 紧凑迷你面板" width="380"/> | <img src="frontend-draft/uploads/PromptPanel-settings.png" alt="PromptPanel 设置：快捷键、主题、粘贴行为、开机启动" width="380"/> |
...
- **网络访问**只剩下 Sparkle 检查更新（仅拉签名 feed，可在设置里关）
- **没有账号体系**：根本没有可登录的东西
...
├── docs/                 # 公开架构、部署、运维、交接文档
├── llms.txt              # 面向 AI 搜索 / LLM 的简短项目索引
├── llms-full.txt         # 面向答案引擎的完整项目索引
└── .github/workflows/    # macOS 发布预检 CI
```
...
面向 AI 搜索引擎和仓库感知工具，优先读取 [llms.txt](llms.txt) 或更完整的 [llms-full.txt](llms-full.txt)。

## Roadmap

PromptPanel 走的是**刻意收敛**的路线。PRD 已经把"永不做"列出来了（云同步、团队、复杂工作流）。在范围内：

- [x] v0.1 — 主链路完成：快捷键 → 搜索 → 执行、项目、剪贴板兜底、明暗、开机启动、Sparkle、签名 + 公证脚本
- [ ] 一键"重复执行上一条词条"
- [ ] JSON / Markdown 导入导出
- [ ] 变量模板（`{{name}}`）—— 仅当不拖累主链路时才做

已发布内容看 [CHANGELOG.md](CHANGELOG.md)，规划中的看 [issues](https://github.com/tytsxai/PromptPanel/issues)。

## 常见问题

更长的 FAQ 见 [FAQ.md](FAQ.md)，这里挑常被问到的：
...
不会。唯一的网络调用是 Sparkle 检查更新（只拉 release 元数据，可在设置里关）。Prompt 正文从不离开你的 Mac。
...
不会，这是有意决定。这些项在 [PRD §4.2](项目快贴-PRD.md) 里被列为**永不做**。产品的核心身份就是"单用户、纯本地、快"，加了那些就不是这个产品了。
...
欢迎 PR——开始前请先看 [CONTRIBUTING.md](CONTRIBUTING.md)。两条不太显然的规则：
...
<sub>**关键词**（方便你搜到）：macOS Prompt 管理 · AI Prompt 启动器 · ChatGPT Prompt 管理 macOS · Claude Prompt 库 · Cursor 代码片段管理 · 开源 TextExpander 替代 · Espanso 替代 · Raycast 替代 · 全局快捷键粘贴 · 本地优先 Prompt 库 · 原生 Swift NSPanel 应用 · AI 工作流效率工具 · Prompt 模板管理 macOS · macOS 片段启动器 · 键盘优先 Prompt 选择器 · 项目快贴.</sub>
